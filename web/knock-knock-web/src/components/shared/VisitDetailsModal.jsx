...
import api, { generateVisitQr } from "@api/apiServices";
...
export default function VisitDetailsModal({
  ...
  const qrImageUrl = (visitState && visitState.qrImageUrl) || visit.qrImageUrl;

  const handleGenerateQR = async () => {
    setIsGenerating(true);
    try {
      const updatedVisit = await generateVisitQr(visit.visitId);
      setVisitState(updatedVisit);
      if (onGenerateQR) {
        onGenerateQR(updatedVisit);
      }
      toast.success("QR code generated successfully! A copy was sent to your email.");
    } catch (error) {
