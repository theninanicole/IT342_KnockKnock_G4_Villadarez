...
import { X, Upload, FileCheck, AlertCircle, FileText } from "lucide-react";
import { fetchCondos, createVisit } from "../services/apiServices";
...
export default function NewVisitModal({ isOpen, onClose, onVisitCreated }) {
  ...
  // Fetch condos on component mount
  useEffect(() => {
    if (isOpen) {
      fetchCondosList();
    }
  ...
  const fetchCondosList = async () => {
    try {
      ...
      console.log("Condos fetched:", data);
  ...
  const handleFileSelect = async (e) => {
    ...
    setUploadedFile({
      name: file.name,
      file: file, // Store the file object for upload on submit
      type: file.type,
    });
    
    console.log("[NewVisitModal] File selected (will upload on submit):", file.name);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.condoId || !formData.unitNumber || !formData.dateOfVisit || !formData.purposeOfVisit) {
    ...
    setLoading(true);
    try {
      // Step 1: Create visit to get real visitId
      console.log("[NewVisitModal] Step 1: Creating visit...");
      const submitData = {
      ...
      console.log("[NewVisitModal] Step 1 Complete - Visit created with ID:", realVisitId);
      toast.success("Visit created!");

      // Step 2: Upload file with real visitId
      console.log("[NewVisitModal] Step 2: Uploading file...");
      setFileLoading(true);
      
      const fileUploadResult = await uploadFile(uploadedFile.file, realVisitId);
      
      console.log("[NewVisitModal] Step 2 Complete - File uploaded:", fileUploadResult);
  ...
  const isFormFilled = formData.condoId && formData.unitNumber && formData.dateOfVisit && formData.purposeOfVisit;
  const isFileSelected = !!uploadedFile;
  const isSubmitDisabled = !isFormFilled || !isFileSelected || loading || fileLoading;
    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 font-sans">
      ...
      <div className="max-w-[420px] w-full bg-white rounded-[24px] shadow-2xl ring-1 ring-gray-900/5 flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 pb-2 overflow-y-auto flex-1">
          <form id="new-visit-form" onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="pt-2">
              ...
              {/* Selected State - File Ready to Submit */}
              {uploadedFile && !fileError && (
              ...
              {fileError && (
                <div className="p-4 border border-red-200 rounded-2xl bg-red-50/50">
                  ...
                  <button
                    type="button"
                    onClick={() => fileInputRef.current?.click()}
                    className="w-full px-4 py-2 text-sm font-medium text-red-600 bg-white border border-red-200 rounded-xl hover:bg-red-50 transition-colors shadow-sm"
                  >
                    Try Again
                  </button>
                </div>
        ...
        <div className="px-6 pb-6 pt-5 border-t border-gray-50 flex-shrink-0 bg-white rounded-b-[24px]">
          <button
            type="submit"
            form="new-visit-form" // Links the button outside the form tag back to the form
