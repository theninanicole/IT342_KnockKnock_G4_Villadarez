...
import { fetchCondos, createVisit } from "@lib/apiServices";
import { uploadFile } from "@features/visits/api/fileService";
...
export default function NewVisitModal({ isOpen, onClose, onVisitCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const hasFieldErrors = Object.values(errors).some((v) => v);
    if (hasFieldErrors) {
      setFieldErrors({
        condoId: errors.condoId ? "Required" : "",
        unitNumber: errors.unitNumber ? "Required" : "",
        dateOfVisit: errors.dateOfVisit ? "Required" : "",
        purposeOfVisit: errors.purposeOfVisit ? "Required" : "",
      });
    ...
    const selectedDate = new Date(`${formData.dateOfVisit}T00:00:00`);
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    if (selectedDate < today) {
      setFieldErrors((prev) => ({
        ...prev,
        dateOfVisit: "Date must be today or later",
      }));
