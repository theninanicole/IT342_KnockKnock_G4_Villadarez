...
import { registerAdminWithGoogle } from "@lib/apiServices";
    ...
    async (event) => {
      ...
      if (userRole === "admin" && googleAdminStep && googleIdToken) {
        try {
          ...
          toast.error(errorMessage?.toLowerCase().includes("exists") ? "This email is already registered." : "Registration failed. Please try again.");
        }
        return;
      }
