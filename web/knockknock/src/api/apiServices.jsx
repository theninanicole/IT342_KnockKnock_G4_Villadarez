export const cancelVisit = async (visitId) => {
  try {
    ...
    console.error("Error cancelling visit - Full error:", error);
    throw error;
  }
};
...
export const sendVisitQrEmail = async (visitId) => {
  try {
    const response = await api.post(`/visits/${visitId}/qr/email`);
    return response.data;
  } catch (error) {
    console.error("Error sending visit QR email - Full error:", error);
    throw error;
  }
};
