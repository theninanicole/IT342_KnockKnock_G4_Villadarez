...
export const uploadFileToSupabase = async (file, visitId) => {
  try {
    ...
    if (error) {
      ...
      throw new Error("We couldn't upload your file. Please check your connection or try again.");
    }
...
export const saveFileMetadata = async (fileMetadata) => {
  try {
    ...
    if (!fileMetadata?.visitId) {
      throw new Error("visitId is required to save file metadata");
    }

    const response = await api.post(`/visits/${fileMetadata.visitId}/files`, fileMetadata);
...
export const deleteFileFromSupabase = async (filePath) => {
  try {
    ...
    if (error) {
      ...
      throw new Error("We couldn't delete your file. Please try again.");
    }
...
export const deleteFileComplete = async (filePath, fileId) => {
  ...
  try {
    ...
    throw new Error("We couldn't delete your file from storage. Please try again.");
  }

  // Step 2: delete DB row via backend
  try {
    console.log("[deleteFileComplete] Calling DELETE /visits/{visitId}/files/" + fileId);
    // NOTE: Deleting by fileId only here; visitId is not required by backend logic
    const response = await api.delete(`/visits/dummy-visit-id/files/${fileId}`);
    ...
    throw new Error("We couldn't remove the file record. Please try again.");
  }
