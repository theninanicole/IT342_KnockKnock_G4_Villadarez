...
import edu.cit.villadarez.knockknock.features.visit.UpdateVisitRequest;
import edu.cit.villadarez.knockknock.features.visit.VisitService;
import edu.cit.villadarez.knockknock.shared.storage.FileStorageService;
import edu.cit.villadarez.knockknock.shared.storage.StorageUploadResult;
...
import edu.cit.villadarez.knockknock.features.visit.VisitFileRepository;
...
public class VisitController {
    ...
    public ResponseEntity<?> createVisit(
        ...
        try {
            ...
            if (idFile != null && !idFile.isEmpty()) {
                try {
                    String folderPath = "visitors_id/" + visit.getVisitId();
                    StorageUploadResult uploadResult = fileStorageService.uploadFile(idFile, folderPath);
                    String fileType = idFile.getContentType() != null ? idFile.getContentType() : "application/octet-stream";
                    String fileName = idFile.getOriginalFilename() != null ? idFile.getOriginalFilename() : "visitor_id";
                    ...
                    System.out.println("[POST /visits] File uploaded to Supabase: " + uploadResult.getPublicUrl());
                } catch (IOException e) {
                    System.err.println("[POST /visits] File upload failed: " + e.getMessage());
                    throw new ResponseStatusException(HttpStatus.BAD_GATEWAY, "Visit was created but ID upload failed: " + e.getMessage());
                }
            }
    ...
    public ResponseEntity<?> getMyVisits() {
        ...
        List<java.util.Map<String, Object>> visitDtos = visits.stream().map(visit -> {
            ...
            dto.put("purpose", visit.getPurpose());
            dto.put("status", visit.getStatus());
            dto.put("qrImageUrl", visit.getQrImageUrl());
            return dto;
        }).toList();
