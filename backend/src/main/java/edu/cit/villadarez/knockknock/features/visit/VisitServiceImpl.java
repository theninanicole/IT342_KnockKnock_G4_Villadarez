...
import edu.cit.villadarez.knockknock.features.visit.VisitStatusChangedEvent;
import edu.cit.villadarez.knockknock.features.visit.UpdateVisitRequest;
import edu.cit.villadarez.knockknock.features.visit.VisitRepository;
...
public class VisitServiceImpl implements VisitService {
    ...
    @Override
    public Visit cancelVisit(UUID visitId, UUID userId) {
        ...
        if (visit.getVisitor() == null || !visit.getVisitor().getId().equals(userId)) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "You do not have permission to cancel this visit");
        }

        // Only allow cancelling if visit is still SCHEDULED
        if (!"SCHEDULED".equalsIgnoreCase(visit.getStatus())) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Only scheduled visits can be cancelled");
        }
